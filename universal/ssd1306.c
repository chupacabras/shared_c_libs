 ...
 * @param  obj:       Pointer to a SSD1306_Handle structure that contains
 *                    the information for the display.
 * @param  c: command byte.
 */
static void SSD1306_command(SSD1306_Handle * obj, uint8_t c) {
 ...
 */
static void SSD1306_command_list(SSD1306_Handle * obj, uint8_t * c, uint8_t count) {
	// 0b00000000 = control byte; Co (continuation bit) = 0, D/C# = 0
 ...
 */
//static bool SSD1306_pixel_within_screen(SSD1306_Handle * obj, uint16_t x, uint16_t y) {
//	return ((x < obj->trans_lcd_width) && (y < obj->trans_lcd_height));
//}

static bool SSD1306_pixel_within_buffer(SSD1306_Handle * obj, int16_t x, int16_t y) {
	return ((x>=0) && (y>=0) && (x < obj->trans_buf_width) && (y < obj->trans_buf_height));
}
...
void SSD1306_init(SSD1306_Handle * obj, uint8_t * buffer, uint8_t *font, uint8_t buf_pix_width, uint8_t buf_pix_height, SSD1306_ChargePump chargepump, SSD1306_Module module, uint8_t i2c_address, device_write_ptr write_cmd) {
	...
	obj->buffer = buffer;
	obj->buf_pix_width = buf_pix_width;
	obj->buf_pix_height = buf_pix_height;
	obj->module = module;
	...
	obj->buf_bytes = (buf_pix_width * (uint16_t) buf_pix_height / 8);
...
void SSD1306_display_buffer(SSD1306_Handle * obj) {
	...
	obj->write_cmd(obj, 0b01000000, obj->buffer, obj->buf_bytes);
...
void SSD1306_display_buffer_at(SSD1306_Handle * obj, uint8_t x, uint8_t y) {
	SSD1306_display_buffer_partial_at(obj, x, y, obj->trans_buf_width, obj->trans_buf_height);
}
...
//	SSD1306_display_buffer_partial(obj, x, y, obj->buf_pix_width, obj->buf_pix_height);
	
//	uint8_t w=obj->trans_buf_width;
//	uint8_t h=obj->trans_buf_height;
	
	if ((obj->trans_lcd_width - x) < w) {
		w = obj->trans_lcd_width - x;
	}
	if ((obj->trans_lcd_height - y) < h) {
		h = obj->trans_lcd_height - y;
	}
	...
	if (obj->rotation == SSD1306_ROTATION_0) {
		...
		trans_y = obj->lcd_height - y - 1 - (h - 1);
		trans_w = w;
		trans_h = h;
	...
	for (i = 0; i <= (page_end-page_start); i++) {
		// 0b01000000 = control byte; Co (continuation bit) = 0, D/C# = 1
		obj->write_cmd(obj, 0b01000000, &obj->buffer[i * obj->buf_pix_width], trans_w);
	}

//	obj->write_cmd(obj, 0b01000000, &obj->buffer[0], obj->buf_bytes);
...
void SSD1306_display_buffer_partial(SSD1306_Handle * obj, uint8_t x, uint8_t y, uint8_t w, uint8_t h) {
	...
	if (!SSD1306_pixel_within_buffer(obj, trans_x, trans_y)) return;
	...
	for (i = page_start; i <= page_end; i++) {
		// 0b01000000 = control byte; Co (continuation bit) = 0, D/C# = 1
		obj->write_cmd(obj, 0b01000000, &obj->buffer[i * obj->buf_pix_width + trans_x], trans_w);
	}
...
void SSD1306_set_rotation(SSD1306_Handle * obj, SSD1306_Rotation rotation) {
	...
	if (obj->rotation == SSD1306_ROTATION_0 || obj->rotation == SSD1306_ROTATION_180) {
		obj->trans_buf_height = obj->buf_pix_height;
		obj->trans_buf_width = obj->buf_pix_width;
		
		obj->trans_lcd_height = obj->lcd_height;
		obj->trans_lcd_width = obj->lcd_width;
	} else {
		obj->trans_buf_height = obj->buf_pix_width;
		obj->trans_buf_width = obj->buf_pix_height;
 ...
 */
static void SSD1306_draw_pixel_unchecked(SSD1306_Handle * obj, uint8_t x, uint8_t y, bool white) {
	uint16_t i;
	...
	if (obj->rotation == SSD1306_ROTATION_0) {
		...
		trans_x = obj->buf_pix_width - y - 1;
		trans_y = x;
	} else if (obj->rotation == SSD1306_ROTATION_180) {
		trans_x = obj->buf_pix_width - x - 1;
		trans_y = obj->buf_pix_height - y - 1;
	} else if (obj->rotation == SSD1306_ROTATION_270_CW) {
		trans_x = y;
		trans_y = obj->buf_pix_height - x - 1;
	}

	i = trans_x + (uint16_t) (trans_y >> 3) * obj->buf_pix_width;
...
void SSD1306_draw_pixel(SSD1306_Handle * obj, int16_t x, int16_t y, bool white) {
	if (!SSD1306_pixel_within_buffer(obj, x, y)) return;

	SSD1306_draw_pixel_unchecked(obj, (uint8_t)x, (uint8_t)y, white);
}
...
void SSD1306_draw_line(SSD1306_Handle * obj, int16_t x1, int16_t y1, int16_t x2, int16_t y2, bool white) {
	...
	while (x <= end) {
		if (x >= 0 && y >= 0) {
			if (rev) {
				if (!SSD1306_pixel_within_buffer(obj, y, x)) break;
				SSD1306_draw_pixel_unchecked(obj, (uint8_t)y, (uint8_t)x, white);
			} else {
				if (!SSD1306_pixel_within_buffer(obj, x, y)) break;
				SSD1306_draw_pixel_unchecked(obj, (uint8_t)x, (uint8_t)y, white);
			}
		}
...
void SSD1306_draw_horizontal_line(SSD1306_Handle * obj, uint8_t x, uint8_t y, uint8_t w, bool white) {
	uint8_t q;
	if (!SSD1306_pixel_within_buffer(obj, x, y)) return;
	for (q = x; q < (((x + w) >= obj->trans_buf_width) ? obj->trans_buf_width : (x + w)); q++) {
...
void SSD1306_draw_vertical_line(SSD1306_Handle *obj, uint8_t x, uint8_t y, uint8_t h, bool white) {
	...
	if (!SSD1306_pixel_within_buffer(obj, x, y)) return;
	for (q = 0; q < h; q++) {
		if (q>=obj->trans_buf_height) break;
		if ((q+y)>=obj->trans_lcd_height) break;
		SSD1306_draw_pixel_unchecked(obj, x, y+q, white);
	}
}
...
void SSD1306_fill_rect(SSD1306_Handle * obj, uint8_t x, uint8_t y, uint8_t w, uint8_t h, bool white) {
	...
	for (i = x; (i < x + w) && (i<=255); i++) {
		SSD1306_draw_vertical_line(obj, (uint8_t)i, y, h, white);
	}
}
 ...
 * @param  white: true = write white pixel, false = write black pixel.
 */
static void circle_draw_pixels(SSD1306_Handle * obj, int16_t xc, int16_t yc, int16_t x, int16_t y, bool white) {
 ...
 */
void SSD1306_draw_circle(SSD1306_Handle * obj, int16_t x0, int16_t y0, uint16_t r, bool white) {
	int16_t x = 0;
	int16_t y = (int16_t)r;
 ...
 * @param  white: true = write white pixel, false = write black pixel.
 */
static void circle_fill_pixels(SSD1306_Handle * obj, int16_t xc, int16_t yc, int16_t x, int16_t y, bool white) {
