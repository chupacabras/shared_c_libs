 ...
 * @param  handle:       Pointer to a DS1302_Handle structure that contains
 ...
 */
void DS1302_init(DS1302_Handle *handle, device_write_pin_ptr write_ce, device_write_pin_ptr write_clk, device_write_pin_ptr write_io, device_read_pin_ptr read_io,
		device_delay_us_ptr delay_us, device_write_pin_ptr set_io_output) {
	handle->write_ce = write_ce;
	handle->write_clk = write_clk;
	handle->write_io = write_io;
	handle->read_io = read_io;
	handle->delay_us = delay_us;
	handle->set_io_output = set_io_output;

	handle->write_ce(false);
}
 ...
 */
static void DS1302_write_byte(DS1302_Handle *handle, uint8_t data) {
	uint8_t i;
	handle->set_io_output(true);

	for (i = 0; i < 8; ++i) {
		handle->write_io(data & 0x01);

		handle->write_clk(false);
		handle->delay_us(2);
		handle->write_clk(true);
		handle->delay_us(2);
		data >>= 1;
	}
}
 ...
 */
static uint8_t DS1302_read_byte(DS1302_Handle *handle) {
	uint8_t i, data;
	handle->set_io_output(false);
 ...
 *                    the information for the display.
 */
static void DS1302_restart_communication(DS1302_Handle *handle) {
	handle->write_clk(false);
	handle->write_ce(false);
	handle->write_ce(true);
	handle->delay_us(4);
 ...
 *                    the information for the display.
 */
static void DS1302_close_communication(DS1302_Handle *handle) {
	handle->write_clk(false);
	handle->write_ce(false);
...
void DS1302_write(DS1302_Handle *handle, uint8_t reg, uint8_t data) {
	DS1302_restart_communication(handle);
	DS1302_write_byte(handle, reg);
	DS1302_write_byte(handle, data);
	DS1302_close_communication(handle);
}
...
void DS1302_write_ram_burst(DS1302_Handle *handle, uint8_t *buf, uint8_t len) {
	DS1302_restart_communication(handle);
	DS1302_write_byte(handle, DS1302_REG_RAM_BURST_W);
	for (temp = 0; temp < len; temp++) {
		DS1302_write_byte(handle, buf[temp]);
	}
	DS1302_close_communication(handle);
}
...
uint8_t DS1302_read_ram(DS1302_Handle *handle, uint8_t offset) {
	...
	DS1302_restart_communication(handle);
	DS1302_write_byte(handle, d);
	d = DS1302_read_byte(handle);
	DS1302_close_communication(handle);
...
void DS1302_read_ram_burst(DS1302_Handle *handle, uint8_t *buf, uint8_t len) {
	DS1302_restart_communication(handle);
	DS1302_write_byte(handle, DS1302_REG_RAM_BURST_R);
	for (temp = 0; temp < len; temp++) {
		buf[temp] = DS1302_read_byte(handle);
	}
	DS1302_close_communication(handle);
}
...
//	temp = DS1302_read(handle, DS1302_REG_SEC_R);
...
//	temp = DS1302_read(handle, DS1302_REG_MIN_R);
...
//	temp = DS1302_read(handle, DS1302_REG_HOUR_R);
...
//	temp = DS1302_read(handle, DS1302_REG_DAY_R);
...
//	temp = DS1302_read(handle, DS1302_REG_DATE_R);
...
//	temp = DS1302_read(handle, DS1302_REG_MONTH_R);
...
//	temp = DS1302_read(handle, DS1302_REG_YEAR_R);
...
//	DS1302_get_seconds(handle, time);
//	DS1302_get_minutes(handle, time);
//	DS1302_get_hours(handle, time);
//	DS1302_get_day_of_week(handle, time);
//	DS1302_get_date(handle, time);
//	DS1302_get_month(handle, time);
//	DS1302_get_year(handle, time);

	DS1302_restart_communication(handle);
	DS1302_write_byte(handle, DS1302_REG_CLOCK_BURST_R);
...
void DS1302_write_time(DS1302_Handle *handle, RTC_time *time) {
	...
	DS1302_write(handle, DS1302_REG_W_PROTECT_W, 0);

	DS1302_restart_communication(handle);
	DS1302_write_byte(handle, DS1302_REG_CLOCK_BURST_W);

	// eight bytes must be written

	temp=num_to_bcd(time->second & 0x7f);
	DS1302_write_byte(handle, temp);
...
//	DS1302_write(handle, DS1302_REG_SEC_W, num_to_bcd(val & 0x7f));
//}
//
///**
...
//	DS1302_write(handle, DS1302_REG_MIN_W, num_to_bcd(val & 0x7f));
//}
//
///**
...
// */
//void DS1302_set_hours(DS1302_Handle *handle, uint8_t val, RTCHoursFormat hour_format) {
//	temp=rtc_encode_hours(val, hour_format);
//	DS1302_write(handle, DS1302_REG_HOUR_W, temp);
//
//}
//
/**
...
void DS1302_set_day_of_week(DS1302_Handle *handle, RTCDay val) {
	DS1302_write(handle, DS1302_REG_DAY_W, val & 0x07);
}
//
///**
...
//	DS1302_write(handle, DS1302_REG_DATE_W, num_to_bcd(val & 0x3f));
//}
//
///**
...
//	DS1302_write(handle, DS1302_REG_MONTH_W, num_to_bcd(val & 0x1f));
//}
//
/**
...
void DS1302_set_year(DS1302_Handle *handle, uint8_t val) {
	DS1302_write(handle, DS1302_REG_YEAR_W, num_to_bcd(val));
}
...
void DS1302_clock_halt(DS1302_Handle *handle) {
	temp = DS1302_read(handle, DS1302_REG_SEC_R);
	temp = temp | 0x80;
	DS1302_write(handle, DS1302_REG_SEC_W, temp);
}
...
void DS1302_set_trickle_charge(DS1302_Handle *handle, DS1302_TrickleCharge trickle) {
	DS1302_write(handle, DS1302_REG_TRICKE_CHARGE_W, trickle);
}
...
DS1302_TrickleCharge DS1302_get_trickle_charge(DS1302_Handle *handle) {
	temp = DS1302_read(handle, DS1302_REG_TRICKE_CHARGE_R);
	return temp;
}
...
//	DS1302_write(handle, DS1302_REG_W_PROTECT_W, write_protect_on?0x80:0);
//}

//uint8_t DS1302_get_write_protect(DS1302_Handle *handle) {
//	return DS1302_read(handle, DS1302_REG_W_PROTECT_R);
//}
